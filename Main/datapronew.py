...
patient_data.drop(patient_data[np.isnan(patient_data['WeanTime'])].index, inplace=True)
...
for pid in pid_lab:
    ...
    this_pid_record_data = record_data[record_data.patient_id==pid].sort_values(['record_time'])
    ...
    this_pid_record_data = this_pid_record_data[this_pid_record_data.record_time < weaningtime]
    this_pid_record_data = this_pid_record_data[this_pid_record_data.record_time > intubationtime]
...
col_float = ['HR','SpO2','DBP','SBP','MBP','P/F比值','乳酸','二氧化碳分压','二氧化碳总量'
         ,'体温','全血剩余碱','全血葡萄糖','总氧饱和度','总胆红素','氧分压','氯','渗透压'
         ,'碳酸氢根浓度','离子钙','红细胞压积','给氧浓度','肺泡动脉氧分压','血色素'
         ,'还原型血红蛋白溶解度'
         ,'酸碱度','钠','钾','高铁血红蛋白'
         ,'呼吸周期(次)','呼吸频率(b/min)','PEEP(cmH2O)'
         ,'吸气时间(s)','压力上升百分比(%)','吸气压力(cmH2O)','PS(cmH2O)','Esens(%)'
         ,'Vsens(L/min)','Psens(cmH2O)','C_STAT(ml/cmH2O)','C_DYN(ml/cmH2O)'
         ,'C_STAT_self(ml/cmH2O)','DP','DP_self'
         ,'Ppeak(cmH2O)','Ppeep(cmH2O)','RR(b/min)','V_Ti(mL)','V_Te(mL)'
         ,'Fipeak(L/min)','Fepeak(L/min)','Ve_TOT(L/min)','WOB_A(J/L)'
         ,'WOB_B(J/L)','MP_area(J/min)','MP_formula(J/min)','IEE','异步率1(%)'
         ,'DT','异步率2(%)','Short Cycle','异步率3(%)','Prolonged Cycle','异步率4(%)','Async Index(%)'
         ]
...
for pid in pid_lab:
    print(pid)
    this_pid_data=data_tmp[data_tmp.patient_id==pid].sort_values(['record_time'])
    basic_data=this_pid_data[col_basic].drop_duplicates().iloc[0].values.tolist()
...
for j in range(0,5):
    ...
    params = {
                ...
                'n_estimators': range(100, 6000, 100),
    ...
    xlf = XGBClassifier(silent=False,   # =0打印信息
                    ...
                    min_child_weight=1,   # 这个参数非常影响结果，控制叶子节点中二阶导的和的最小值，该参数值越小，越容易 overfitting。
    ...
    gsearch = RandomizedSearchCV(xlf, param_distributions=params, scoring='accuracy', cv=3)
    
    gsearch.fit(X_train_kfold, Y_train_kfold, early_stopping_rounds=10,  eval_set=eval_set, verbose=10)

    params=gsearch.best_params_
    clf = XGBClassifier(silent=False,   # =0打印信息
                        ...
                        min_child_weight=params['min_child_weight'],   # 这个参数非常影响结果，控制叶子节点中二阶导的和的最小值，该参数值越小，越容易 overfitting。
                        gamma=params['gamma'],   # 树的叶子节点上作进一步分区所需的最小损失减少,越大越保守，一般0.1、0.2这样子。
                        reg_alpha=params['reg_alpha'],
                        # reg_lambda=0.01,
                        nthread=4,  # cpu线程数
                        
                        scale_pos_weight=params['scale_pos_weight'],  # 如果取值大于0的话，在类别样本不平衡的情况下有助于快速收敛。平衡正负权重
                        seed=27,  # 随机种子
                        eval_metric=['logloss','auc','error'])  

    clf.fit(X_train_kfold, Y_train_kfold, early_stopping_rounds=10, eval_set=eval_set, verbose=10)
#   删选important_features
    feature_names=X_train_kfold.columns
    feature_importance=pd.DataFrame(pd.Series(clf.get_booster().get_fscore()),columns=['importance_of_'+str(k)+'_fold'])

    important_features=feature_importance.loc[feature_importance['importance_of_'+str(k)+'_fold']>0,:].index
    ...
    clf.fit(X_train_final, Y_train_kfold, early_stopping_rounds=50, eval_set=eval_set, verbose=10)
...
#
    ...
    num=50
    plt.barh(range(len(feature_importances[-num:])),feature_importances[-num:].importance, tick_label=feature_importances[-num:].feature,color=feature_importances[-num:].color)
    plt.xticks(size = 20)
    plt.yticks(size = 20)
    plt.tight_layout()
